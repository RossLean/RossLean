...
public class DuplicateAttributeArgumentRemover : GACodeFixProvider
{
    ...
    protected override async Task<Document> PerformCodeFixActionAsync(
        CodeFixContext context, SyntaxNode syntaxNode, CancellationToken cancellationToken)
    {
        ...
        argument.GetAttributeRelatedParents(
            out var argumentList, out var attribute, out var attributeList);

        var typeParameter = attributeList.Parent as TypeParameterSyntax;
        var attributes = typeParameter.AttributeLists
